...
data "aws_iam_policy_document" "analyzer_queue_policy" {
  statement {
    ...
    // Allow the BinaryAlert bucket and any configured external_s3_buckets to notify the SQS queue.
    condition {
      test     = "ArnEquals"
      variable = "aws:SourceArn"
      values = ["${concat(list(aws_s3_bucket.binaryalert_binaries.arn), var.external_s3_bucket_resources)}"]
    }
  }
}

// Allow analyzer queue to be sent messages from the BinaryAlert S3 bucket and other configured external_s3_buckets.
